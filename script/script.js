    ...
    fetch(`${url}/suggest/${search_term}`)
        ...
        .then(data => {
            ...
            if(data.data.length == 0){
                item_div.innerHTML ="No Song Found Try Again"
            }
            else{
                populate(data)
        ...
        .catch(
            setTimeout(()=>{item_div.innerHTML ="loding...."}, 3)
...
//fetch lyric by this function LyricApi
    ...
    fetch(`https://api.lyrics.ovh/v1/${artist_name}/${title}`)
        ...
        .then(data => {
            ...
            if (data.lyrics === undefined) {
                item_div.innerHTML = `<h2><span id="header_artist">${artist_name}</span><br> Lyric Not Found</h2> <a href="">Home</a> `
            } else {
